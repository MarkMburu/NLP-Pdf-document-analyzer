...
def get_sections(pages):
    ...
    for page_num, page in enumerate(pages):
        ...
        for ind, i in enumerate(clean_page):
            ...
            if re.findall("^Section \d+", i) and "page" not in i or (re.sub("\d+ [\w+\s+]+", "",i)=="" and ind==0 and len(i) > 6):
...
def get_similar_sentences(df_1, df_2):
    ...
    cv.fit(pd.concat([df_1['Sentance'], df_2['Sentance']]))

    df_1_feat = pd.DataFrame(cv.transform(df_1['Sentance']).toarray(), index=df_1['Sentance'].index)
    df_2_feat = pd.DataFrame(cv.transform(df_2['Sentance']).toarray(), index=df_2['Sentance'].index)
    ...
    for i,j in  score.idxmin().loc[mins].items():
        try:
            results.append((df_1['Sentance'].loc[i], df_1['Page'].loc[i], df_2['Sentance'].loc[j],df_2['Page'].loc[j]))
        except:
            pass

    return pd.DataFrame(results, columns=["File 1 Sentance", "File 1 Page", "File 2 Sentance", "File 2 Page"])
